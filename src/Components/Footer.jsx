...
function Footer() {
  ...
  return (
    <div data-scroll data-scroll-speed=".2" className='flex flex-col p-4 md:p-20'>
      ...
      <div className='flex flex-col md:flex-row gap-8 justify-between'>
        ...
        {/* Right - Company Info and Socials */}
        <div className='w-full md:w-1/2 flex flex-col gap-6'>
          <p className='text-base md:text-lg leading-relaxed text-zinc-700 max-w-md'>
            We are a creative agency specializing in eye-opening presentations that transform ideas into compelling visual stories. From startups to Fortune 500 companies, we help brands communicate with clarity and impact.
          </p>
          <div className='flex flex-wrap gap-4 md:gap-6'>
            {['Instagram', 'Facebook', 'Twitter', 'LinkedIn'].map((item, index) => (
              <a 
                key={index} 
                href="#" 
                className='text-base md:text-lg font-medium relative group'
              >
                {item}
      ...
      <motion.div 
        className='w-full mt-12 md:mt-20 rounded-xl overflow-hidden cursor-hover'
        animate={{ 
          backgroundColor: isCollabHovered ? '#ffffff' : '#000000'
        }}
        transition={{ duration: 0.4, ease: [0.4, 0, 0.2, 1] }}
        onMouseEnter={() => setIsCollabHovered(true)}
        onMouseLeave={() => setIsCollabHovered(false)}
      >
        <div className='flex items-center justify-between px-6 md:px-12 py-6 md:py-10'>
          <motion.h2 
            className='text-2xl md:text-4xl lg:text-6xl font-bold tracking-tight'
            animate={{ color: isCollabHovered ? '#000000' : '#ffffff' }}
            transition={{ duration: 0.4, ease: [0.4, 0, 0.2, 1] }}
          >
            Let's Collaborate
          </motion.h2>
          ...
          <motion.div 
            className='p-3 md:p-4 rounded-full border-2'
            animate={{ 
              borderColor: isCollabHovered ? '#000000' : '#ffffff'
            }}
            transition={{ duration: 0.4, ease: [0.4, 0, 0.2, 1] }}
          >
            <div className="relative w-6 h-6 md:w-8 md:h-8 overflow-hidden">
              <motion.div
                animate={{ y: isCollabHovered ? '-100%' : '0%' }}
              ...
              >
                <motion.div
                  ...
                  transition={{ duration: 0.4, ease: [0.4, 0, 0.2, 1] }}
                >
                  <RiArrowRightUpLine className='w-6 h-6 md:w-8 md:h-8'/>
                </motion.div>
              </motion.div>
              <motion.div
              ...
              >
                <motion.div
                  ...
                  transition={{ duration: 0.4, ease: [0.4, 0, 0.2, 1] }}
                >
                  <RiArrowRightDownLine className='w-6 h-6 md:w-8 md:h-8'/>
                </motion.div>
              </motion.div>
            </div>
          </motion.div>
      ...
      {/* Copyright */}
      <div className='mt-8 md:mt-12 pt-6 border-t border-zinc-300 flex flex-col md:flex-row justify-between items-center gap-4'>
        <p className='text-sm text-zinc-500'>© 2024 Oochi. All rights reserved.</p>
        <div className='flex gap-6'>
          <a href="#" className='text-sm text-zinc-500 hover:text-black transition-colors'>Privacy Policy</a>
          <a href="#" className='text-sm text-zinc-500 hover:text-black transition-colors'>Terms of Service</a>
        </div>
      </div>
    </div>
  );
